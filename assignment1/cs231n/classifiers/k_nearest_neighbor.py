...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    # pass
    # diff_sq = (self.X_train - X[:, None, :])**2
    # diff_sq = np.square(self.X_train - X[:, None, :])
    # dists = np.sqrt(np.sum(diff_sq, axis=2))
    dists = np.sqrt(np.sum(np.square(self.X_train - X[:, None, :]), axis=2))
    # print(np.shape(diff_sq))
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      #########################################################################
      # pass
      # idx_closet_y = np.argsort(dists, axis=1)[:, 0:k]
      idx_y = np.argpartition(dists, k, axis=1)  #idx_y is a num_test*num_train array
      #print('idx_y type is', type(idx_y))      
      #print('idx_y shape is', np.shape(idx_y))
      for idx_closest_y in range(num_test):
        closest_y = self.y_train[idx_y[idx_closest_y, :],] 
      ...
      # counts_closest_y = collections.Counter(closest_y)
      # y_pred = counts_closest_y.most_common(1)[0][0]
      for idx_y_pred in range(num_test):
        counts_closest_y = collections.Counter(closest_y)
        y_pred = counts_closest_y.most_common(1)[0][0]
