...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # Forward into first layer (hidden layer)
    hidden_layer, cache_hidden_layer = affine_relu_forward(X, W1, b1)
    # Forward into second layer (output layer)
    ...
    # gradient calculation with backpropogation
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    # Unpack input parameters to self attributs
    #self.D = input_dim
    #self.H = hidden_dims
    #self.C = num_classes
    ...
    for idx_layer in range(self.num_layers):
      self.params['b%d' % (idx_layer + 1)] = np.zeros(dims[idx_layer + 1])
      self.params['W%d' % (idx_layer + 1)] = np.random.randn(dims[idx_layer], dims[idx_layer + 1]) * weight_scale
    ...
    if self.use_dropout:
      ...
      print('Dropout is used with p = %f' % (self.dropout_param['p']))
      if seed is not None:
    ...
    if self.use_batchnorm:
      ...
      print('Batchnorm is used for training.')
  ...
  def loss(self, X, y=None):
    ...
    # Forward into layer[1] to layer[end-1]
    for idx_layer in range(1, self.num_layers):
      layer[idx_layer], cache_layer[idx_layer] = affine_relu_forward(layer[idx_layer - 1],
                                                                     self.params['W%d' % idx_layer],
                                                                     self.params['b%d' % idx_layer])
    # Forwaard into last layer
    ...
    ############################################################################
    #pass
    # loss after Softmax layer
    data_loss, dscores = softmax_loss(scores, y)
    # loss regularization
    reg_loss = 0
    for idx in range(1, self.num_layers + 1):
      reg_loss += 0.5 * self.reg * np.sum(self.params['W%d'% idx]**2)
    loss = data_loss + reg_loss
    ...
    # Backprop into layer[end-1] to layer[1]
    for idx in reversed(range(1, self.num_layers)):
      dx[idx], grads['W%d' % idx], grads['b%d' % idx] = affine_relu_backward(dx[idx + 1], cache_layer[idx])
